        ...
        <div :key="isFormSubmitted || errorMessage">
          <form v-if="!isFormSubmitted && !errorMessage" id="transfer-form" @submit.prevent="submitForm" :class="{ 'form-transition': !errorMessage }">
            <div class="input-container">
              ...
              <input type="text" id="sourceAccount" name="contaOrigem" v-model="sourceAccount" placeholder="Digite a conta de origem" class="bottom-border" required />
            </div>
            <div class="input-container">
              <label for="destinationAccount">Conta de Destino</label>
              <input type="text" id="destinationAccount" name="contaDestino" v-model="destinationAccount" placeholder="Digite a conta de destino" class="bottom-border" required />
            </div>
            <div class="input-container">
              <label for="amount">Saldo</label>
              <input type="text" id="amount" name="saldo" v-model="amount" placeholder="Digite o saldo" class="bottom-border" @input="validateAmount" required />
            </div>
            <div class="input-container">
              <label for="transferDate">Data de Transferência</label>
              <input type="date" id="transferDate" name="dataTransferencia" v-model="transferDate" class="bottom-border" required />
            </div>
            <div class="input-container">
...
export default {
  ...
  methods: {
    submitForm() {
      ...
      const transferDateObject = new Date(this.transferDate);
      const day = transferDateObject.getUTCDate().toString().padStart(2, '0');
      const month = (transferDateObject.getUTCMonth() + 1).toString().padStart(2, '0');
      const year = transferDateObject.getUTCFullYear();

      const transferDateFormatted = `${day}/${month}/${year}`;
